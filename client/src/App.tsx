...
function App() {
  ...
  const displayEvidence: DisplayEvidence[] = overview?.evidence.map((item, index) => ({ label: item.kind, detail: item.detail, status: item.status, time: 'chain', icon: evidence[index].icon })) ?? evidence
  ...
  const capabilityState = paused ? 'PAUSED' : overview?.capability.status ?? 'ACTIVE'
  ...
  const simulate = async (blocked: boolean) => {
    ...
    try {
      ...
      setSimulation(blocked ? 'blocked' : 'allowed')
      setSimulationReason('control plane unavailable; showing local fixture result')
    }
  }
