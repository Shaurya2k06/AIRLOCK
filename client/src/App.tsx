...
function DemoPage({ onHome }: { onHome: () => void }) {
  ...
  useEffect(() => {
    let mounted = true
    fetchJson<Overview>(`${API_URL}/api/overview`)
      .then((value) => { if (mounted) { setOverview(value); setLoadError(false) } })
      .catch(() => { if (mounted) setLoadError(true) })
    fetchJson<Runbook>(`${API_URL}/api/runbook`)
      .then((value) => { if (mounted) setRunbook(value) })
      .catch(() => { if (mounted) setRunbook(null) })
    fetchJson<Protocol>(`${API_URL}/api/protocol`)
      .then((value) => { if (mounted) setProtocol(value) })
      .catch(() => { if (mounted) setProtocol(null) })
    fetchJson<ReleasePassport>(`${API_URL}/api/release-passport`)
      .then((value) => { if (mounted) setPassport(value) })
