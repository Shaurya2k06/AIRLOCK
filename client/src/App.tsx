...
type Overview = {
  ...
  policy: { recipient: string; maxPayment: number }
  actions: Array<{ action: string; target: string; amount: number; state: string; age: string }>
}

type DisplayEvidence = { label: string; detail: string; time?: string; status: string; icon: IconName }
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:8787'}/api/overview`)
      .then((response) => response.ok ? response.json() : Promise.reject(new Error('control plane unavailable')))
      .then((value: Overview) => { if (mounted) setOverview(value) })
      .catch(() => undefined)
    return () => { mounted = false }
  }, [])

  const displayEvidence: DisplayEvidence[] = overview?.evidence.map((item, index) => ({ label: item.kind, detail: item.detail, status: item.status, time: 'chain', icon: evidence[index]?.icon ?? 'box' })) ?? evidence
  const displayActivity = overview?.actions?.length ? overview.actions.map((item) => ({ ...item, amount: `$${item.amount.toFixed(2)}`, state: item.state.includes('Allowed') ? 'Allowed' : 'Blocked' })) : activity
  const release = overview?.release ?? { name: 'Atlas-7b', version: '1.4.2', digest: '0x8f24c1a7…d83a71', status: 'ACTIVE', expiresAt: '08:41 UTC' }
  const provenEvidence = displayEvidence.filter((item) => item.status === 'PROVEN').length
  const network = overview?.network ?? { source: 'Ethereum Sepolia', destination: 'Creditcoin CC3', chainKey: 'pending' }
  const liveData = overview?.dataSource === 'creditcoin-chain'
  const localPaused = !liveData && paused
  const capabilityState = localPaused ? 'PAUSED' : overview?.capability.status ?? 'ACTIVE'
  ...
  const simulate = async (blocked: boolean) => {
    const recipient = blocked ? '0x0000000000000000000000000000000000000001' : overview?.policy.recipient ?? '0x4E…91c2'
    const amount = blocked ? 24 : Math.min(24, overview?.policy.maxPayment ?? 250)
    try {
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div className="release-mini"><div className="release-mini-icon"><Icon name="box" size={16} /></div><div className="release-mini-copy"><strong>{release.name}</strong><span>v{release.version} · {release.status.toLowerCase()}</span></div><span className="mini-check"><Icon name="check" size={12} /></span></div>
      ...
      <main className="main-content">
        <header className="topbar"><div className="breadcrumb"><span>Northstar Labs</span><Icon name="chevron" size={14} /><strong>{activeNav}</strong></div><div className="topbar-actions"><div className="live-indicator"><span className="status-dot" /> {liveData ? 'chain data' : 'local fixture'}</div><button className="icon-button" aria-label="Open proof inspector"><Icon name="terminal" size={17} /></button><button className="wallet-chip"><span className="wallet-avatar"><Icon name="wallet" size={14} /></span><span>{overview?.capability.runtimeKey ?? '0x8B…6A14'}</span><Icon name="chevron" size={14} /></button></div></header>

        {activeNav === 'Overview' && <div className="page-body">
          <section className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" /> CONTROL PLANE / RELEASE 01</div><h1>Release firewall</h1><p>Cryptographic evidence decides what your agent can do.</p></div><div className="heading-actions"><button className="secondary-button"><Icon name="external" size={15} /> View on Blockscout</button><button className="primary-button" disabled={liveData} title={liveData ? 'Use the guardian runbook for live emergency controls' : undefined} onClick={() => { if (!liveData) setPaused(!paused) }}><Icon name={liveData ? 'shield' : paused ? 'play' : 'pause'} size={15} /> {liveData ? 'Guardian control in runbook' : paused ? 'Resume local fixture' : 'Pause local fixture'}</button></div></section>
          <section className="metric-grid" aria-label="Release metrics"><div className="metric-card metric-highlight"><div className="metric-top"><span>Release state</span><Icon name="shield" size={18} /></div><div className="metric-value status-value"><span className={`large-dot ${localPaused ? 'paused' : ''}`} /> {capabilityState}</div><div className="metric-foot">{liveData ? 'Read from Creditcoin' : 'Local fixture only'}</div></div><div className="metric-card"><div className="metric-top"><span>Evidence</span><span className="metric-accent">{provenEvidence.toString().padStart(2, '0')} / 04</span></div><div className="metric-value">{liveData ? (provenEvidence === 4 ? 'VERIFIED' : 'PENDING') : 'FIXTURE'}</div><div className="metric-foot">One digest across all proofs</div></div><div className="metric-card"><div className="metric-top"><span>Spend remaining</span><span className="metric-accent">{overview ? `${Math.max(0, Math.round(((overview.capability.spendCap - overview.capability.spent) / Math.max(overview.capability.spendCap, 1)) * 100))}%` : '63%'}</span></div><div className="metric-value">{overview ? `$${Math.max(0, overview.capability.spendCap - overview.capability.spent).toFixed(2)}` : '$220.00'}</div><div className="meter"><span style={{ width: `${overview ? Math.max(0, Math.min(100, ((overview.capability.spendCap - overview.capability.spent) / Math.max(overview.capability.spendCap, 1)) * 100)) : 63}%` }} /></div><div className="metric-foot">of ${overview?.capability.spendCap.toFixed(2) ?? '350.00'} total cap</div></div><div className="metric-card"><div className="metric-top"><span>Revocation gap</span><Icon name="clock" size={17} /></div><div className="metric-value">{liveData ? 'bounded' : '08:42'}</div><div className="metric-foot">Maximum detection window</div></div></section>

          <section className="release-layout"><div className="panel release-panel"><div className="panel-header"><div><div className="panel-kicker">AUTHORIZED RELEASE</div><h2>{release.name} <span>/</span> v{release.version}</h2></div><div className="verified-badge"><Icon name="check" size={13} /> {liveData ? release.status : 'FIXTURE'}</div></div><div className="digest-row"><span className="digest-label">releaseDigest</span><code>{release.digest}</code><button className="copy-button" aria-label="Copy release digest"><Icon name="copy" size={14} /></button></div><div className="release-divider" /><div className="release-details"><div><span>Runtime key</span><strong><span className="key-dot" /> {overview?.capability.runtimeKey ?? '0x8B31…6A14'}</strong></div><div><span>Policy</span><strong>northstar-agent-v1</strong></div><div><span>Issued</span><strong>{liveData ? 'chain import' : 'Today, 08:31 UTC'}</strong></div><div><span>Expires</span><strong>{release.expiresAt}</strong></div></div><div className="source-chain"><span className="chain-icon eth">◆</span><span>{network.source}</span><Icon name="arrow" size={16} /><span className="chain-icon cc">C</span><span>{network.destination}</span><span className="chain-confirm"><Icon name="check" size={12} /> {liveData ? 'proof verified' : 'fixture data'}</span></div></div>
            <div className="panel boundary-panel"><div className="panel-header"><div><div className="panel-kicker">CAPABILITY BOUNDARY</div><h2>What the agent can do</h2></div><span className="live-pill"><span className="status-dot" /> {liveData ? 'chain' : 'fixture'}</span></div><div className="boundary-list"><div className="boundary-row"><div className="boundary-icon green"><Icon name="wallet" size={16} /></div><div><strong>Vendor payment</strong><span>1 registered recipient</span></div><b>≤ ${overview?.policy.maxPayment ?? 250}</b></div><div className="boundary-row"><div className="boundary-icon amber"><Icon name="box" size={16} /></div><div><strong>Protocol deposit</strong><span>Bounded service call</span></div><b>≤ $100</b></div><div className="boundary-row"><div className="boundary-icon purple"><Icon name="activity" size={16} /></div><div><strong>Call budget</strong><span>Nonces are single-use</span></div><b>{overview?.capability.callCap ?? 2} calls</b></div></div><div className="boundary-note"><Icon name="lock" size={14} /> No raw transaction access · no vault withdrawal</div></div></section>

          <section className="panel evidence-panel"><div className="panel-header evidence-heading"><div><div className="panel-kicker">EVIDENCE GRAPH</div><h2>Four independent proofs. One release.</h2></div><button className="text-button" onClick={() => setActiveNav('Evidence graph')}>Open inspector <Icon name="arrow" size={14} /></button></div><div className="evidence-track">{displayEvidence.map((item, index) => <div className="evidence-item" key={item.label}><div className="evidence-node"><Icon name={item.icon} size={18} /><span className="node-check"><Icon name="check" size={10} /></span></div><div className="evidence-copy"><strong>{item.label}</strong><span>{item.detail}</span><small>{item.time ?? 'chain' } · <b>{item.status}</b></small></div>{index < displayEvidence.length - 1 && <div className="evidence-connector"><span /></div>}</div>)}</div><div className="evidence-footer"><span><span className="status-dot" /> {liveData ? 'All proofs read from Creditcoin for ' : 'Fixture digest '}<code>{release.digest}</code></span><span>{liveData ? 'source chain key ' : 'Attestation latency '}<b>{liveData ? network.chainKey : '2m 11s'}</b></span></div></section>

          <section className="bottom-grid"><div className="panel action-panel"><div className="panel-header"><div><div className="panel-kicker">ACTION GATE</div><h2>Test the boundary</h2></div><span className="action-state"><span className="status-dot" /> signer ready</span></div><p className="panel-description">The model proposes. AIRLOCK validates the exact intent before the vault can move value.</p><div className="intent-preview"><div className="intent-line"><span>tool</span><code>vendor.pay</code><span className="intent-allow">ALLOWLISTED</span></div><div className="intent-line"><span>recipient</span><code>{overview?.policy.recipient ?? '0x4E9b…91c2'}</code><span>$24.00</span></div><div className="intent-line"><span>nonce</span><code>01</code><span>deadline 60s</span></div></div><div className="action-buttons"><button className="primary-button" onClick={() => simulate(false)}><Icon name="play" size={15} /> Simulate allowed call</button><button className="danger-button" onClick={() => simulate(true)}><Icon name="shield" size={15} /> Test blocked call</button></div>{simulationCopy && <div className={`simulation-result ${simulationCopy.tone}`}><span className="result-icon"><Icon name={simulationCopy.tone === 'success' ? 'check' : 'shield'} size={15} /></span><div><strong>{simulationCopy.title}</strong><span>{simulationCopy.body}</span></div><button onClick={() => { setSimulation('idle'); setSimulationReason('') }} aria-label="Dismiss result">×</button></div>}</div>
        ...
        {activeNav !== 'Overview' && <DetailView activeNav={activeNav} onBack={() => setActiveNav('Overview')} evidenceItems={displayEvidence} capability={overview?.capability} activity={displayActivity} />}
      </main>
    </div>
  )
}

function DetailView({ activeNav, onBack, evidenceItems, capability, activity: displayActivity }: { activeNav: string; onBack: () => void; evidenceItems: DisplayEvidence[]; capability?: Overview['capability']; activity: Array<{ action: string; target: string; amount: number | string; state: string; age: string }> }) {
  ...
  return <div className="detail-page"><div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" /> AIRLOCK / INSPECTOR</div><h1>{titles[activeNav]}</h1><p>Verified state from the current release firewall.</p></div><button className="secondary-button" onClick={onBack}><Icon name="arrow" size={15} /> Back to overview</button></div>
    {activeNav === 'Evidence graph' && <div className="detail-grid">{evidenceItems.map((item) => <div className="panel detail-card" key={item.label}><div className="detail-card-top"><div className="evidence-node"><Icon name={item.icon} size={18} /><span className="node-check"><Icon name="check" size={10} /></span></div><span className="verified-badge"><Icon name="check" size={12} /> {item.status}</span></div><h2>{item.label}</h2><p>{item.detail}</p><code>{item.time ?? 'chain data'}</code><div className="detail-meta"><span>Ethereum Sepolia</span><span>{item.status === 'PROVEN' ? 'receipt status 1' : 'not verified'}</span></div></div>)}</div>}
    {activeNav === 'Capabilities' && <div className="panel capability-detail"><div className="capability-summary"><div className="capability-lock"><Icon name="lock" size={24} /></div><div><div className="panel-kicker">CAPABILITY ID</div><h2>chain capability</h2><p>Non-transferable authority bound to <code>{capability?.runtimeKey ?? '0x8B31…6A14'}</code></p></div><span className="verified-badge"><Icon name="check" size={12} /> {capability?.status ?? 'FIXTURE'}</span></div><div className="capability-grid"><div><span>Scope root</span><strong>chain-bound</strong></div><div><span>Total spend</span><strong>${capability?.spent.toFixed(2) ?? '130.00'} / ${capability?.spendCap.toFixed(2) ?? '350.00'}</strong></div><div><span>Calls used</span><strong>{capability?.callsUsed ?? 2} / {capability?.callCap ?? 4}</strong></div><div><span>Valid until</span><strong>status evidence</strong></div></div><div className="boundary-note"><Icon name="shield" size={14} /> Every action is revalidated against current release status.</div></div>}
    {activeNav === 'Action log' && <div className="panel full-table"><div className="panel-header"><div><div className="panel-kicker">CHAIN-BOUND EVENTS</div><h2>All enforcement decisions</h2></div><span className="live-pill"><span className="status-dot" /> indexed</span></div><div className="expanded-table">{displayActivity.concat([{ action: 'vault.withdraw', target: 'direct call', amount: '—', state: 'Blocked', age: '2h ago' }]).map((row) => <div className="expanded-row" key={`${row.action}-${row.age}`}><span className="row-time">{row.age}</span><div><strong>{row.action}</strong><small>{row.target}</small></div><span>{typeof row.amount === 'number' ? `$${row.amount.toFixed(2)}` : row.amount}</span><b className={row.state === 'Allowed' ? 'allowed' : 'blocked'}><span />{row.state}</b><Icon name="external" size={15} /></div>)}</div></div>}
  </div>
}
