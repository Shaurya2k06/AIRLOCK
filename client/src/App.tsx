...
function LiveProtocolSurface({ overview, passport, protocol, onEnterDemo }: { overview: Overview | null; passport: ReleasePassport | null; protocol: Protocol | null; onEnterDemo: () => void }) {
  const proven = overview?.evidence.filter((item) => item.status === 'PROVEN' || item.status === 'REVOKED').length ?? 0
  ...
  const passportChecks = passport?.passportVerification?.checks
  return <section className="product-section product-container live-protocol-section" id="live-protocol">
    ...
    <div className="live-protocol-grid">
      <div className="live-protocol-card"><span>RELEASE PASSPORT</span><strong>{passport?.verified ? 'VERIFIED' : 'UNAVAILABLE'}</strong><code>{passport?.releaseDigest ?? '—'}</code><small>{passport ? `v${passport.releaseVersion} · ${passport.fileCount} files · OCI ${passport.passportVerification?.checks.oci?.status ?? 'not-configured'}` : 'No verified passport loaded'}</small></div>
      ...
      <div className="live-protocol-card"><span>TEE RUNTIME BINDING</span><strong>{protocol?.runtimeAssurance.level === 'L2' ? 'VERIFIED' : 'NOT CONFIGURED'}</strong><code>{shortHash(protocol?.runtimeAssurance.binding?.measurement)}</code><small>{protocol?.runtimeAssurance.binding ? `SNP measurement · quote ${shortHash(protocol.runtimeAssurance.binding.quoteHash)}` : 'No verifier-attested runtime binding'}</small></div>
      <div className="live-protocol-card"><span>CREDENTIAL DELEGATION</span><strong>{protocol?.delegation.registry ? 'ON-CHAIN' : 'UNAVAILABLE'}</strong><code>{protocol?.delegation.registry ? shortHash(protocol.delegation.registry) : '—'}</code><small>EIP-712 and ERC-1271 credentials can only narrow tools, budget, calls, risk, and lifetime.</small></div>
      <div className="live-protocol-card"><span>DECISION PROTOCOLS</span><strong>MCP · A2A · x402</strong><code>ALLOW · AUTH_REQUIRED · DENY</code><small>Tool discovery, task authorization, and payment settlement stay behind the same live capability.</small></div>
      <div className="live-protocol-card"><span>BUILD ATTESTATIONS</span><strong>{passport?.passportVerification?.verified ? 'VERIFIED' : 'INCOMPLETE'}</strong><code>{['sbom', 'provenance', 'sigstore', 'rekor'].map((name) => passportChecks?.[name]?.status === 'verified' ? name : '').filter(Boolean).join(' · ') || '—'}</code><small>OCI is reported separately so a registry access failure cannot be mistaken for a verified pull.</small></div>
      <div className="live-protocol-card"><span>IDENTITY / TRACE</span><strong>{protocol?.identity.configured ? `ERC-8004 #${protocol.identity.agentId}` : 'UNAVAILABLE'}</strong><code>{protocol?.identity.agentRegistry ?? '—'}</code><small>Release passport, runtime assurance, evidence roots, and action trace roots remain inspectable.</small></div>
    </div>
    <div className="live-protocol-footer"><span>{protocol ? `${protocol.mcp.protocolVersion} · credential attenuation · recursive revocation · live tool revalidation` : 'Connect the control plane to inspect live protocol state'}</span><button className="section-link" onClick={onEnterDemo}>Open live controls <Icon name="arrow" size={14} /></button></div>
  </section>
}
...
function LandingPage({ onEnterDemo }: { onEnterDemo: () => void }) {
  ...
  return <div className="product-landing">
    ...
    <main>
      ...
      <section className="product-section product-container attestcoin-section" id="attestcoin"><div className="section-intro section-intro-wide"><p className="product-kicker">ATTESTCOIN VERIFICATION LAYER</p><h2>Source-chain facts, verified where authority is enforced.</h2><p>For the current supported path, events originate on Ethereum Sepolia, Attestcoin proves inclusion and continuity, and the pinned receipt decoder plus AIRLOCK adapter validate semantics before normalized evidence reaches the Creditcoin authority path.</p></div><ArchitectureDiagram /><div className="attestcoin-notes"><div><strong>Receipt truth</strong><span>Status, chain key, emitter, topic count, data length, log index, schema, and decoded fields are checked on Creditcoin.</span></div><div><strong>Deterministic authority</strong><span>Policy, evidence, runtime binding, capability issuance, delegation, and router checks narrow authority before execution.</span></div><div><strong>Worker liveness</strong><span>Workers discover, batch, and submit proofs. They do not decide truth or issue capabilities.</span></div></div><div className="architecture-caption"><code>Sepolia registries → Attestcoin → decoder + adapter → evidence + policy → capability → router → vault</code><a href="https://github.com/Shaurya2k06/AIRLOCK/blob/main/docs/attestcoin-integration.md" target="_blank" rel="noreferrer">Read the integration notes <Icon name="external" size={13} /></a></div></section>

      <section className="product-section product-container policy-section" id="policy"><div className="section-intro"><p className="product-kicker">POLICY STUDIO</p><h2>Turn governance into a machine-checkable boundary.</h2><p>Teams configure what evidence is acceptable and what the runtime may do after the conjunction succeeds.</p></div><div className="policy-studio"><div className="policy-sidebar"><span>POLICY / ON-CHAIN</span><b>Approval policy</b><a className="active" href="#policy">Evidence requirements</a><a href="#capability">Capability limits</a><a href="#execution">Execution boundary</a><a href="#revocation">Guardian response</a></div><div className="policy-form"><div className="policy-form-header"><div><span>DEPLOYED STATE</span><h3>Release policy</h3></div><span className="policy-status"><i /> {landingOverview ? 'read from chain' : 'open live demo'}</span></div><div className="policy-fields"><label><span>Approved recipient</span><strong>{landingOverview?.policy.recipient ?? 'chain policy'}</strong></label><label><span>Per-call ceiling</span><strong>{landingOverview ? `${landingOverview.policy.maxPayment} native` : 'policy-defined'}</strong></label><label><span>Deposit ceiling</span><strong>{landingOverview?.policy.depositMax ? `${landingOverview.policy.depositMax} native` : 'policy-defined'}</strong></label><label><span>Evidence quorum</span><strong>policy-defined</strong></label><label><span>Capability TTL</span><strong>policy-defined</strong></label><label><span>Status freshness</span><strong>policy-defined</strong></label></div><div className="policy-tags"><span>scope intersection</span><span>recipient allowlist</span><span>nonce required</span><span>budget accounting</span><span>{landingProtocol?.runtimeAssurance.level === 'L2' ? 'TEE binding verified' : 'TEE optional'}</span></div></div></div><a className="section-link" href="/demo" onClick={(event) => { event.preventDefault(); onEnterDemo() }}>Open the policy surface <Icon name="arrow" size={14} /></a></section>

      <section className="product-section product-container capability-section" id="capability"><div className="section-intro"><p className="product-kicker">CAPABILITY CENTER</p><h2>Authority is visible, bounded, and easy to kill.</h2><p>Once evidence agrees, AIRLOCK issues a non-transferable capability bound to the runtime key. On-chain child capabilities can only narrow tools, budget, calls, risk, and lifetime; revoking a parent disables its descendants.</p></div><div className="capability-card"><div className="capability-card-top"><div><span className="capability-live"><i /> {liveCapability?.status ?? 'CHAIN-BACKED CAPABILITY'}</span><h3>{liveCapability?.id ? 'runtime authority' : 'inspect the live capability'}</h3></div><span className="capability-shield"><Icon name="lock" size={18} /></span></div><div className="capability-card-digest"><span>releaseDigest</span><code>{liveDigest}</code></div><div className="capability-metrics"><div><span>Runtime key</span><strong>{liveCapability?.runtimeKey ?? '—'}</strong></div><div><span>Scope root</span><strong>{liveCapability?.scopeRoot ?? '—'}</strong></div><div><span>Spend remaining</span><strong>{liveCapability ? `${Math.max(0, liveCapability.spendCap - liveCapability.spent)} native` : '—'}</strong></div><div><span>Calls remaining</span><strong>{liveCapability ? `${Math.max(0, liveCapability.callCap - liveCapability.callsUsed)} / ${liveCapability.callCap}` : '—'}</strong></div><div><span>Expiry</span><strong>{liveCapability?.expiresAt ?? '—'}</strong></div><div><span>Policy hash</span><strong>on-chain policy</strong></div></div><div className="capability-card-footer"><span><Icon name="check" size={13} /> non-transferable</span><span><Icon name="check" size={13} /> revalidated per action</span><span><Icon name="shield" size={13} /> revocation-aware</span></div></div></section>
    ...
    <footer className="product-footer product-container" id="access"><div><p className="product-kicker">READY WHEN YOUR AGENT TOUCHES REAL VALUE</p><h2>Protect the next release your agent ships.</h2><p>Run AIRLOCK in your own environment, inspect the architecture, or start with the live proof surface.</p></div><div className="product-footer-actions"><a className="product-button product-button-secondary" href="https://github.com/Shaurya2k06/AIRLOCK/blob/main/docs/architecture.md" target="_blank" rel="noreferrer">Read the architecture <Icon name="external" size={14} /></a><button className="product-button product-button-ghost" onClick={onEnterDemo}>Open live demo <Icon name="arrow" size={14} /></button></div></footer>
  </div>
}
function App() {
