...
function App() {
  ...
  const displayEvidence: DisplayEvidence[] = overview?.evidence.map((item, index) => ({ label: item.kind, detail: item.detail, status: chainUnavailable ? 'UNAVAILABLE' : item.status, time: 'chain', icon: evidence[index]?.icon ?? 'box', txHash: item.txHash, creditcoinTxHash: item.creditcoinTxHash, sourceBlock: item.sourceBlock, logIndex: item.logIndex, receiptStatus: item.receiptStatus })) ?? evidence
