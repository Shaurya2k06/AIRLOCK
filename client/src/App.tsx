...
function DemoPage({ onHome }: { onHome: () => void }) {
  ...
  const issueCredential = async () => {
    ...
    if (runbook?.writeAuthRequired && !operatorToken) {
      setProtocolMessage('operator token required; no credential was issued')
      return
    }
    ...
    try {
      ...
      setCredentialToken(`Bearer AirlockCredential ${encoded}`)
      setProtocolMessage('credential issued; copy it into the agent runtime Authorization header')
    } catch (error) {
      setProtocolMessage(error instanceof Error ? error.message : 'credential issuance failed')
    }
  }
