...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:8787'}/api/overview`)
      .then((response) => response.json().then((value: Overview) => { if (mounted) setOverview(value); return value }))
      .catch(() => undefined)
    return () => { mounted = false }
  }, [])

  const liveData = overview?.dataSource === 'creditcoin-chain'
  const chainUnavailable = overview?.dataSource === 'rpc-error'
  const displayEvidence: DisplayEvidence[] = overview?.evidence.map((item, index) => ({ label: item.kind, detail: item.detail, status: chainUnavailable ? 'UNAVAILABLE' : item.status, time: 'chain', icon: evidence[index]?.icon ?? 'box' })) ?? evidence
  ...
  const capabilityState = chainUnavailable ? 'UNAVAILABLE' : localPaused ? 'PAUSED' : overview?.capability.status ?? 'ACTIVE'
