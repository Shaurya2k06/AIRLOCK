  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeNav === 'Live' && <LiveRunPanel state={liveRunState} steps={liveRunSteps} message={liveRunMessage} overview={displayOverview} writesEnabled={Boolean(runbook?.writesEnabled)} onRun={runLive} />}
...
function LiveRunPanel({ state, steps, message, overview, writesEnabled, onRun }: { state: 'idle' | 'running' | 'complete' | 'failed'; steps: LiveRunStep[]; message: string; overview: Overview | null; writesEnabled: boolean; onRun: () => void }) {
...
function LiveProtocolSurface({ overview, passport, protocol, onEnterDemo }: { overview: Overview | null; passport: ReleasePassport | null; protocol: Protocol | null; onEnterDemo: () => void }) {
  ...
  return <section className="product-section product-container live-protocol-section" id="live-protocol">
    ...
    <div className="live-protocol-grid">
      ...
      <div className="live-protocol-card"><span>ASSURANCE / TRACE</span><strong>{protocol ? `${protocol.runtimeAssurance.level} ${protocol.runtimeAssurance.name}` : 'UNAVAILABLE'}</strong><code>{protocol?.identity.configured ? 'ERC-8004 identity verified' : 'ERC-8004 adapter available'}</code><small>Release passport, runtime assurance, evidence roots, and action trace roots remain inspectable.</small></div>
    </div>
